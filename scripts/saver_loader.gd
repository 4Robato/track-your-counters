...
const CUSTOM_TRACKERS_PATH : String = Global.PATH_SETTINGS + "cust_trackers.save"
...
func save_UI_size():
	...
	saved_data["language_selected"] = Global.language_selected
...
func load_settings():
	...
	while save_file.get_position() < save_file.get_length():
		...
		if node_data.has("language_selected"):
			Global.language_selected = node_data["language_selected"]
		else:
			Global.language_selected = "en"
...
func save_current_trackers(file_name : String):
	...
	for item in Global.main_menu.all_trackers.get_children():
		if item is Tracker:
	...
	saved_data["all_logs"] = Global.logs_bbcode
...
func save_custom_trackers():
	var file = FileAccess.open(CUSTOM_TRACKERS_PATH, FileAccess.WRITE)
	var all_trackers : Array[Dictionary]
	var saved_data : Dictionary[String, Variant] = {}
	
	for item in Global.tracker_manager.trackers.get_children():
		if item is PreviewTracker:
			all_trackers.append(item.t_info.get_dict())
...
func load_custom_trackers() -> void:
	if not FileAccess.file_exists(CUSTOM_TRACKERS_PATH):
		return # Error! We don't have a save to load.
	
	var save_file = FileAccess.open(CUSTOM_TRACKERS_PATH, FileAccess.READ)
	
	Global.tracker_manager.delete_all_trackers()
	...
	while save_file.get_position() < save_file.get_length():
		...
		for i in num_trackers:
			...
			Global.tracker_manager.add_custom_tracker(tracker)
